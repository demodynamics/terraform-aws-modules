...
resource "aws_vpc" "vpc" {
  ...
  lifecycle {
    precondition {
      ...
      error_message = "The desired AZ count (${var.az_desired_count}) exceeds the available AZs count (${length(data.aws_availability_zones.this.names)}). Please reduce var.az_desired_count."
    }
  }
}
