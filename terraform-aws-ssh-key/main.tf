...
- The output ssh_private_key_pem provides the private key (sensitive)—save this securely and never 
commit it to version control.

Use aws_key_pair.ssh.key_name as your ssh_key variable, and you’re ready for secure SSH access! 
...
resource "aws_key_pair" "ssh" {
  key_name   = try(var.aws_ssh_key_pair_name, "ec2-ssh-key")
  public_key = tls_private_key.ssh.public_key_openssh
  /*
  The resource aws_key_pair "ssh" uploads the public key (generated by tls_private_key.ssh) to AWS as an EC2 
  key pair.

- The public key is stored in AWS under the name ssh-key.
...
This allows you to reference ssh-key when launching EC2 instances (including EKS nodes), enabling SSH 
...
resource "local_file" "ssh_private_key" {
  ...
  /*
- The SSH private key will be saved as ssh-key.pem in your working directory with secure permissions.
- You can use this file directly to SSH into your EC2 (EKS nodes) (if remote_access block in node group module is 
enabled and security group allows it).

Remember: Keep ssh-key.pem safe and never commit it to version control. If you need further automation or
