# Generating trust policy for IAM Role That would be aasumed by Gitlab CI/CD pipeline

data "aws_iam_policy_document" "gitlab_trust_policy" {
  version = "2012-10-17"
  ...
  statement {
    ...
    principals {
      ...
      identifiers = [var.gitlab_oidc_arn] # Defines that the role with this trust policy can be assumed only by external system (Federated type Web Identity) by token issued by external system OIDC provider
    }

    condition {
      test     = "StringEquals"
      variable = "${replace(var.gitlab_oidc_issuer_url, "https://", "")}:aud"
      values   = ["https://gitlab.com"]
    }

    condition {
      test     = "StringLike"
      variable = "${replace(var.gitlab_oidc_issuer_url, "https://", "")}:sub"
      values   = ["repo:${var.gitlab_group}/${var.gitlab_project}:ref:${var.gitlab_branch == "*" ? "*" : var.gitlab_branch}"] # Allows to assume the role only if the token is issued for the specified Gitlab group, project and branch. If branch is "*", then all branches of the project can assume the role.
    }
  }
}
...
resource "aws_iam_role" "gitlab_ci_role" {
  description        = "Role That will be Assumed by token provided by Gitlab OIDC Provider"
  name               = "Gitlab-${var.gitlab_group}-${replace(var.gitlab_project, "*", "any")}-repo-${replace(var.gitlab_branch, "*", "any")}-branch-role"
  assume_role_policy = data.aws_iam_policy_document.gitlab_trust_policy.json # Adding generated trust policy into role
...
resource "aws_iam_role_policy_attachment" "self_managed_permissions_policy_attachment" {
  count      = length(var.self_managed_policy_permissions) > 0 ? 1 : 0
  role       = aws_iam_role.gitlab_ci_role.name          # The Role Name
