variable "fargate_profile_name" {
  ...
  validation {
    condition = can(regex("^[a-z0-9-]{1,64}$", var.fargate_profile_name))
    error_message = "fargate_profile_name must be between 1 and 64 characters, and can only contain lowercase letters, numbers, and hyphens."
  }
}

variable "policies" {
  description = "List of AWS managed permissions policy(s) for Fargate Profile Role"
  type        = set(string)
